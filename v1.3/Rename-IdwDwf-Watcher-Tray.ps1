...
# Version: v1.3 (lock + housekeeping + status)
#
# Folder layout (versioned):
#   C:\Scripts\v1.3\Rename-IdwDwf-Watcher-Tray.ps1
#   C:\Scripts\v1.3\dwf_rename_settings.json
#   C:\Scripts\v1.3\dwf_rename_watcher.log
#
# Features:
...
$notify.Text = "DWF Auto-Rename v1.3 (starting...)"
$notify.Visible = $true
...
function Set-TrayStatus([string]$status) {
    switch ($status) {
        "starting" { $notify.Icon = $IconStarting; $notify.Text = "DWF Auto-Rename v1.3 (starting...)" }
        "online"   { $notify.Icon = $IconOnline;   $notify.Text = "DWF Auto-Rename v1.3 (online)" }
        "offline"  { $notify.Icon = $IconOffline;  $notify.Text = "DWF Auto-Rename v1.3 (offline)" }
        "mixed"    { $notify.Icon = $IconStarting; $notify.Text = "DWF Auto-Rename v1.3 (partial connectivity)" }
    }
}
...
function Show-StatusWindow {
    ...
    $form.Text = "DWF Auto-Rename Status (v1.3)"
...
function Show-SettingsDialog {
    ...
    $form = New-Object System.Windows.Forms.Form
    $form.Text = "DWF Auto-Rename Settings (v1.3)"
    $form.StartPosition = "CenterScreen"
    $form.Width = 920
    $form.Height = 560
    $form.Topmost = $true
...
# Main loop
# -------------------------
Log "SCRIPT START (v1.3): settings=$script:SettingsPath paths=$($global:DwfWatchPaths -join '; ')"
Set-TrayStatus "starting"
