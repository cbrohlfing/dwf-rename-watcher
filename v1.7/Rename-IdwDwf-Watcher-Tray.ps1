...
# Version: v1.7 (lock + housekeeping + status)
#
# Folder layout (versioned):
#   C:\Scripts\v1.7\Rename-IdwDwf-Watcher-Tray.ps1
#   C:\Scripts\v1.7\dwf_rename_settings.json
#   C:\Scripts\v1.7\dwf_rename_watcher.log
#
# Features:
...
# -------------------------
$script:Version = "v1.7"
$script:AppName  = "DWF Auto-Renamer"
$script:AppLabel = "$script:AppName $($script:Version)"
...
$notify.Text = "$script:AppLabel (starting...)"
$notify.Visible = $true
...
function Set-TrayStatus([string]$status) {
    switch ($status) {
        "starting" { $notify.Icon = $IconStarting; $notify.Text = "$script:AppLabel (starting...)" }
        "online"   { $notify.Icon = $IconOnline;   $notify.Text = "$script:AppLabel (online)" }
        "offline"  { $notify.Icon = $IconOffline;  $notify.Text = "$script:AppLabel (offline)" }
        "mixed"    { $notify.Icon = $IconStarting; $notify.Text = "$script:AppLabel (partial connectivity)" }
    }
}
...
function Try-RenameIdwDwf([string]$path) {
    ...
    try {
        ...
        # Final target name/path
        $newName = $name -replace "\.idw\.dwf$", ".dwf"
        $folder  = Split-Path $path -Parent
        $newPath = Join-Path $folder $newName

        # Lock the TARGET name to prevent multi-PC races
        $lockTargetPath = $newPath
        $lockAcquired = Try-AcquireLock $lockTargetPath
        if (-not $lockAcquired) { return }

        # Wait for incoming copy to finish (file unlock)
        for ($t=0; $t -lt 60; $t++) {
            if (-not (Test-Path -LiteralPath $path)) { return }
            try {
        ...
        # If target .dwf exists, archive it to .\Archive\<base>.<timestamp>.dwf
        if (Test-Path -LiteralPath $newPath) {
            $base = [System.IO.Path]::GetFileNameWithoutExtension($newName)  # e.g. 100004024

            $archiveDir = Join-Path $folder "Archive"
            if (-not (Test-Path -LiteralPath $archiveDir)) {
                try { New-Item -ItemType Directory -Path $archiveDir -Force | Out-Null } catch {}
            }

            $timestamp = (Get-Date).ToString("yyyyMMdd-HHmmss")
            $archivedPath = $null

            # Find an available timestamped name (handle multiple archives same second)
            for ($i = 0; $i -le 999; $i++) {
                $suffix = if ($i -eq 0) { "" } else { "-$i" }
                $archivedName = "{0}.{1}{2}.dwf" -f $base, $timestamp, $suffix
                $candidate = Join-Path $archiveDir $archivedName
            ...
            if (-not $archivedPath) {
                Log "SKIP: Could not find available archive name for '$newName'."
                return
            }

            # Move old .dwf into archive (wait briefly if locked)
            $moved = $false
            for ($t=0; $t -lt 40; $t++) {  # ~10 seconds
                try {
                    Move-Item -LiteralPath $newPath -Destination $archivedPath -Force -ErrorAction Stop
                    $moved = $true
                    Log ("ARCHIVED: '{0}' -> 'Archive\{1}'" -f $newName, (Split-Path $archivedPath -Leaf))
                    break
                } catch {
                    Start-Sleep -Milliseconds 250
                }
            ...
            if (-not $moved -and (Test-Path -LiteralPath $newPath)) {
                Log "SKIP: Could not archive existing '$newName' (file in use or permission issue)."
                return
            }
        }

        # Rename incoming file to final .dwf
        ...
        if ($lockAcquired -and $lockTargetPath) { Release-Lock $lockTargetPath }
    }
}
...
function Show-StatusWindow {
    ...
    $form.Text = "$script:AppLabel - Status"
...
function Show-SettingsDialog {
    ...
    $form = New-Object System.Windows.Forms.Form
    $form.Text = "$script:AppLabel - Settings"
    $form.StartPosition = "CenterScreen"
    $form.Width = 920
    $form.Height = 560
    $form.Topmost = $true
...
# Main loop
# -------------------------
Log "SCRIPT START ($script:Version): settings=$script:SettingsPath paths=$($global:DwfWatchPaths -join '; ')"
Set-TrayStatus "starting"
...
Log "SCRIPT EXIT ($script:Version)"
